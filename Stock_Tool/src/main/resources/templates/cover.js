...
function validateSubmit(info){
   if(!validateCode(info.code)){
    ...
    if(info.type==1&&!validateMakeMoney(info.secPrice)){
        return false;
    }
 ...
 * @returns {boolean}
 */
function validateMakeMoney(buyingPrice){
    //验证想赚的钱数
...
function showInfo(result){
    //展示手续费等信息
    var calcBuyMoneyVoList=result.calcBuyMoneyVoList;
    ...
    $("#buyCharge2").text(calcBuyMoneyVoList[1].buyCharge);
    $("#buyTransferFee2").text(calcBuyMoneyVoList[1].buyTransferFee);
    $("#buyCommunications2").text(calcBuyMoneyVoList[1].buyCommunications);
    $("#dealMoney2").text(calcBuyMoneyVoList[1].dealMoney);
    $("#totalBuyCharge2").text(calcBuyMoneyVoList[1].totalBuyCharge);

    $("#totalBuyCharge").text(result.totalBuyCharge);
    //展示操作的信息
